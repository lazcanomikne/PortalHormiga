  ...
  <v-card class="mb-4" id="datos-basicos" aria-label="Datos Básicos" flat>
    ...
    <v-card-text>
      <v-row v-if="store.articuloActual.itemCode === 'KBK'">
        ...
        <v-col cols="12" md="8">
          <v-text-field v-model="store.datosBasicos.izaje" type="number" min="0"
        ...
        <template v-if="store.datosBasicos.longitudBrazo === 'No estándar'">
          ...
          <v-col cols="12" md="8">
            <v-text-field v-model="store.datosBasicos.longitudBrazoValor" type="number" min="0"
              :rules="[v => v >= 0 || 'No se permite negativos']" placeholder="0 mm" suffix="mm" density="compact" />
          </v-col>
        </template>
      </v-row>
      <v-row v-else-if="store.articuloActual.itemCode === 'Grua giratoria'">
        ...
        <v-col cols="12" md="8">
          <v-select v-model="store.datosBasicos.dispositivoElevacion"
            ...
            @update:model-value="getCodigoContruccion(store.datosBasicos.dispositivoElevacion)" />
        </v-col>
        <v-col cols="12" md="4" class="d-flex align-center">
          <label class="text-body-1">Codigo de construcción</label>
        </v-col>
        <v-col cols="12" md="8">
          <v-combobox v-model="store.datosBasicos.codigoConstruccion" :items="codigoContruccion" :loading="loading"
            density="compact" />
        </v-col>
        <v-col cols="12" md="4" class="d-flex align-center">
      ...
      <v-row v-else-if="store.articuloActual.itemCode === 'Grua giratoria KBK'">
        ...
        <v-col cols="12" md="8">
          <v-select v-model="store.datosBasicos.dispositivoElevacion"
            :items="['Polipasto de Cadena', 'Polipasto de Cable']" density="compact"
            @update:modelValue="getCodigoContruccion(store.datosBasicos.dispositivoElevacion)" />
        </v-col>
        <v-col cols="12" md="4" class="d-flex align-center">
          <label class="text-body-1">Codigo de construcción</label>
        </v-col>
        ...
        <template v-if="store.datosBasicos.altura">
          ...
          <v-col cols="12" md="8">
            <v-text-field v-model="store.datosBasicos.altura" type="number" min="0"
              :rules="[v => v >= 0 || 'No se permite negativos']" placeholder="0 mm" suffix="mm" density="compact"
        ...
        <template v-if="store.datosBasicos.plumaBordeInferior">
          ...
          <v-col cols="12" md="8">
            <v-text-field v-model="store.datosBasicos.plumaBordeInferior" type="number" min="0"
              :rules="[v => v >= 0 || 'No se permite negativos']" placeholder="0 mm" suffix="mm" density="compact"
        ...
        <template v-if="store.datosBasicos.posicionMasAltaGancho">
          ...
          <v-col cols="12" md="8">
            <v-text-field v-model="store.datosBasicos.posicionMasAltaGancho" type="number" min="0"
              :rules="[v => v >= 0 || 'No se permite negativos']" placeholder="0 mm" suffix="mm" density="compact"
      ...
      <v-row v-else>
        ...
        <v-col cols="12" md="8">
          <v-text-field v-model="store.datosBasicos.capacidadGrua" type="number" min="0"
        ...
        <v-col cols="12" md="8">
          <v-text-field v-model="store.datosBasicos.claro" type="number" min="0"
            :rules="[v => v >= 0 || 'No se permite negativos']" placeholder="0 mm" suffix="mm" density="compact"
              ...
              <v-card class="mb-4" flat>
                <v-card-text>
                  <v-row>
                    ...
                    <v-col cols="12" md="8">
                      <v-select v-model="store.datosBasicos.cantidadGanchos" :items="[0, 1, 2, 3]" density="compact" />
                    </v-col>
        ...
        <v-col cols="12" md="8">
          <v-select v-model="store.puente.especifiqueAreaTrabajo"
            :items="['Interior', 'Exterior', 'Polvoroso', 'Polvoroso explosivo', 'Otros']" density="compact" />
        </v-col>
        <template v-if="store.puente.especifiqueAreaTrabajo === 'Otros'">
          <v-col cols="12" md="4" class="d-flex align-center">
            <label class="text-body-1">Especifique área de trabajo</label>
          </v-col>
          <v-col cols="12" md="8">
            <v-text-field v-model="store.puente.especifiqueAreaTrabajoOtro" density="compact" />
          </v-col>
        </template>
        <!-- sección Controles -->
        <v-col cols="12" md="12">
        ...
        <v-col cols="12" md="8">
          <v-text-field v-model="store.datosBasicos.pesoMuertoGrua" type="number" min="0"
            :rules="[v => v >= 0 || 'No se permite negativos']" placeholder="0 kg" suffix="kg" density="compact" />
        </v-col>
        <v-col cols="12" md="4" class="d-flex align-center">
...
watch(() => store.datosBasicos.cantidadGanchos, (newValue) => {
  if (newValue && newValue > 0) {
    ...
    // Ajustar el array según la cantidad seleccionada
    const currentLength = store.datosBasicos.ganchos.length;
    const targetLength = newValue;
    ...
    if (targetLength > currentLength) {
      ...
      store.eliminarPolipasto(targetLength)
    }
  }
}, { immediate: true });
