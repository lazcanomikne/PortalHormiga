...
const crearCotizacion = async () => {
  ...
  const encabezado = {
    ...
    clienteNombre: storeCotizador.form.cliente.nombreCompleto.split(' - ')[1] || '',
    clienteFinal: storeCotizador.form.clienteFinal || '',
    personaContacto: storeCotizador.form.personaContacto || '',
    direccionFiscal: storeCotizador.form.direccionFiscal || '',
    direccionEntrega: storeCotizador.form.direccionEntrega || '',
    referencia: storeCotizador.form.referencia || '',
    terminosEntrega: storeCotizador.form.terminosEntrega.trnspCode || '',
    folioPortal: storeCotizador.form.folioPortal ? parseInt(storeCotizador.form.folioPortal) : null,
    folioSap: storeCotizador.form.folioSAP ? parseInt(storeCotizador.form.folioSAP) : null,
  ...
  try {
    const response = await cotizacionService.create(`"${JSON.stringify(cotizacionCompleta).replace(/"/g, '\\"')}"`);
...
const actualizarCotizacion = async () => {
  ...
  try {
    ...
    console.log('Actualizando cotización:', cotizacionCompleta);
    // Llamar al servicio de actualización
    const response = await cotizacionService.update(storeCotizador.getEditId, `"${JSON.stringify(cotizacionCompleta).replace(/"/g, '\\"')}"`);

    console.log('Cotización actualizada:', response.data);

    // Mostrar mensaje de éxito
    mostrarMensaje('Cotización actualizada exitosamente', 'success');
