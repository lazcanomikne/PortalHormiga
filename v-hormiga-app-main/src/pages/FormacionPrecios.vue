      ...
      <v-card class="mb-4" outlined>
        ...
        <v-card-text>
          <v-row>
            <!-- Plazo de pago -->
            <v-col cols="12" md="12">
              <v-combobox v-model="store.formacionPrecios.plazoPago" label="Plazo de pago" item-title="pymntGroup"
            ...
            <v-col cols="12" md="12" v-if="store.formacionPrecios.cliReqFianza >= 1">
              <v-select v-model="store.formacionPrecios.tipoFianza1" label="Tipo de fianza 1"
                :items="storeArticuloDefiniciones.tipoFianza" item-title="u_Tipodefianza" density="compact"
            ...
            <v-col cols="12" md="6">
              <v-select v-model="store.formacionPrecios.tiempoGarantia" label="Tiempo de garantía"
                :items="storeArticuloDefiniciones.tipoGarantias" item-title="u_Tiempodegarantia" density="compact"
                :loading="loadingData" :rules="[v => !!v || 'Campo obligatorio']" />
            </v-col>
...
const loadTableData = () => {
  ...
  storeArticles.selectedArticles.forEach(item => {
    ...
    if (!hasExtraCost && definiciones.montaje) {
    ...
    if (hasExtraCost) {
      conceptos.push({
        codigo: `Costo_${item.itemCode}`,
        descripcion: `Costo ${item.itemName}`,
        cantidad: 1,
        precioUnitario: 0,
        precioTotal: 0,
        edit: false
...
onMounted(async () => {
  //store.conceptos = []
  console.log(storeArticles.selectedArticles)
  await loadData()
  loadTableData()
})
